  ...
  useEffect(() => {
    ...
    const loadHiveData = async () => {
      try {
        ...
        const foundHive = savedHives.find((h: any) => h.id.toString() === hiveId);
        setHive(foundHive);
        ...
        const hiveInspections = savedInspections
          .filter((inspection: any) => inspection.hive === foundHive?.name)
            ...
            <View style={styles.overviewHeader}>
              ...
              <View style={[styles.statusBadge, { backgroundColor: getStatusColor(hive.status) + '20' }]}>
                <Text style={[styles.statusText, { color: getStatusColor(hive.status) }]}>
                  {hive.status === 'critical' && hive.hasQueen === false ? 'Drottninglöst' : 
            ...
            <View style={styles.statsGrid}>
              ...
              <View style={styles.statCard}>
                ...
                <Text style={[styles.statValue, { 
                  color: parseFloat(hive.varroa || '0') > 5 ? '#E74C3C' : 
                         parseFloat(hive.varroa || '0') > 2 ? '#FF8C42' : '#8FBC8F' 
                }]}>
                  {hive.varroa}
                </Text>
