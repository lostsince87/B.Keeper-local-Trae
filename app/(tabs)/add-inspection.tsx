...
const generateBasicAnalysis = (inspectionData: Inspection): any => {
  ...
  if (inspectionData.varroaPerDay && inspectionData.varroaPerDay > 5) {
  ...
  useEffect(() => {
    const loadHives = async () => {
      try {
        ...
        const hiveNames = savedHives.map((hive: { name: string }) => hive.name);
    ...
    const saveInspection = async () => {
      try {
        ...
        if (newQueenAdded) {
          ...
          const updatedHives = existingHives.map((hive: any) => {
