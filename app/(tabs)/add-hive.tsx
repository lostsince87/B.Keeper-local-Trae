...
export default function AddHiveScreen() {
  ...
  const [queenColor, setQueenColor] = useState<QueenColor | ''>('');
  ...
  const handleSave = () => {
    ...
    const queenData = hasQueen ? {
      hasQueen: true,
      queenMarked: queenMarked || false,
      queenColor: queenMarked ? (queenColor as QueenColor) : undefined,
      queenWingClipped: queenWingClipped || false,
      ...
      queenAddedDate: undefined,
    };

    // Skapa nytt kupobjekt
    const newHive: Hive = {
      id: Date.now().toString(), // Enkel ID-generering som string
      name: hiveName.trim(),
      location: location.trim(),
      type: 'Langstroth', // Default type
      frames: `0/${frameCount}`, // Start with 0 brood frames
      isNucleus: frameCount <= 10 ? (isNucleus || false) : false,
      isWintered: false,
      notes: notes.trim(),
    ...
    const saveHive = async () => {
      try {
        ...
        const nameExists = existingHives.some((hive: { name: string }) =>
                    ...
                    <View style={styles.colorSelector}>
                      {queenColors.map((color) => (
                        <Pressable
                          ...
                          onPress={() => setQueenColor(color.id as QueenColor)}
                        >
