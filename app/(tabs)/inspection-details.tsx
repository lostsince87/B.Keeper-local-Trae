...
export default function InspectionDetailsScreen() {
  ...
  useEffect(() => {
    ...
    const loadInspection = async () => {
      try {
        ...
        const foundInspection = savedInspections.find((i: any) => i.id.toString() === inspectionId);
  ...
  const queenColors: { [key: string]: string } = {
    white: '#FFFFFF',
    yellow: '#FFD700',
    red: '#FF0000',
    green: '#008000',
  ...
  const getVarroaLevelColor = (level: string) => {
    switch (level) {
          ...
          {inspection.varroaPerDay && (
            <View style={styles.card}>
              ...
              <View style={styles.varroaSection}>
                <Bug size={32} color={getVarroaLevelColor(inspection.varroaLevel || 'lågt')} />
                <View style={styles.varroaInfo}>
                  <Text style={[styles.varroaValue, { color: getVarroaLevelColor(inspection.varroaLevel || 'lågt') }]}>
                    {inspection.varroaPerDay.toFixed(1)} varroa/dag
                  </Text>
                  <Text style={[styles.varroaLevel, { color: getVarroaLevelColor(inspection.varroaLevel || 'lågt') }]}>
            ...
            <View style={styles.card}>
              ...
              {inspection.aiAnalysis.recommendations && inspection.aiAnalysis.recommendations.length > 0 && (
                <View style={styles.aiSection}>
                  ...
                  {inspection.aiAnalysis.recommendations.map((rec: string, index: number) => (
              ...
              {inspection.aiAnalysis.priority_actions && inspection.aiAnalysis.priority_actions.length > 0 && (
                <View style={styles.aiSection}>
                  ...
                  {inspection.aiAnalysis.priority_actions.map((action: string, index: number) => (
          ...
          {inspection.observations && inspection.observations.length > 0 && (
            <View style={styles.card}>
              ...
              <View style={styles.observationsDisplay}>
                {inspection.observations.map((obsId: string, index: number) => {
